...
Create reviewable authoring files and generated GitLab CI using the installed CLI. Read [the authoring procedure](references/authoring.md) for onboarding, editing, validation and troubleshooting. This package targets the 0.4 workflow interface; inspect the installed CLI/schema before using fields from memory.
...
3. `depends-on` propagates project selection; `needs` transfers same-event producer artifacts; `release.needs` orders coordinated publication and release completion; deployment waits for completion. `release.gitlab: true` explicitly enables the GitLab Release API. None substitutes package sources. Candidate dependencies are explicit per-run test inputs.
...
6. Edit authoring files, then validate, doctor, explain, render and check drift. Use explain --event/--changed for selection and upgrade for reviewed schema/source/runtime binding updates. Report local compilation separately from live GitLab CI Lint and execution. Preserve the user's branch and existing authorization scope.
