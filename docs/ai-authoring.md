...
This is the entry point for agents creating or changing pipelines with generic-ci 0.4.0. It describes implemented interfaces. The older product proposals and scenario audits are design history; do not infer support from those documents.
...
Those schemas describe delivery/platform configuration. Source manifest/project schemas are bundled separately. Schema format version 1 is not toolkit version 0.4.0 or an organization source tag such as v1.4.0.
...
generic-ci validate
generic-ci doctor
generic-ci explain --json -o /tmp/generic-ci-explain.json
...
For changes, use `explain --event merge-request --changed PATH...` and inspect selection reasons. Use `upgrade` to preview source/schema/image-binding changes together. Package publication, image builds and optional `release.gitlab` have separate effects; production deployments wait for release completion. All workflow role images require Bash and protocol 2 / 0.4.x compatibility.
