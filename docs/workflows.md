...
The 0.4 workflow interface uses team-defined checks and event selection. Start with the [README quick start](../README.md#quick-start). Version 0.3.2 passed the repository’s fast CI and disposable GitLab/Runner E2E scenarios; Buildah, registry publication, and OpenShift rollout still require qualification on the target infrastructure.

## Start and inspect

Install the versioned toolkit wheel and dependencies from your internal package service or image factory. Workflow compiler and runtime use protocol 2 with the compatible 0.4.x runtime series. All role images must contain a compatible runtime; future behavioral/protocol changes require a new contract. Legacy generated pipelines still require exact package versions. No component downloads this repository into consumers.
...
Commit authoring files and generated YAML. The plan job verifies source fingerprints. `render --check -o .gitlab-ci.yml` detects drift locally. `generic-ci explain` prints effective commands and dependencies. Use `--event merge-request --changed services/api/main.py` to simulate selection, or `--json` for effective configuration, origins and structured results.
...
- `workflows` supports push, merge-request, release, manual (GitLab web pipeline), schedule, api, trigger, and pipeline. Checks are explicit lists. Push is suppressed when an MR pipeline takes its place; lists are not implicitly combined.
...
3. `release.needs: [sdk]` orders publication and release completion after the prerequisite project has built and published its configured outputs. Deployment approval and rollout wait for these completion receipts. Tests and builds remain independent unless their own `needs` explicitly require a producer. Currently both projects must use the same coordinated tag convention. Independent cross-repository publication orchestration is not implemented; use candidate validation before coordinated releases or publish the prerequisite first.

Change selection includes declared chart directories and values files automatically. Project `watch` adds shared paths. A common closure drives native GitLab `rules:changes`, runtime selection and local explanation, retaining all required producers and avoiding unaffected jobs/approval buttons. Root workspace manifests/locks or authoring configuration changes select all projects. Native rules conservatively broaden above GitLab’s 50-pattern limit; candidate overrides also defer filtering to the runtime. Missing diff history is fetched; an unreadable known baseline fails the planner instead of demanding receipts from omitted jobs. New branches without a baseline select all projects. A shared generated SDK and API may use separate project identities with the same source directory; directory containment is not project identity.

## Custom execution

Platform defaults merge with project defaults and check fields. Scalars replace, maps merge recursively and lists replace. Omitted fields inherit. There is no null-based removal operator. Checks may set dependencies: false for no environment preparation; Python preparation settings may be overridden per check. Commands use Bash with `-euo pipefail`, preserving shell state between entries and propagating failures through pipes; Windows workflow execution is not supported yet.
...
Python defaults to pyproject.toml project.version; Node defaults to package.json version and SemVer ordering. Explicit file/field selection is supported. A release declaration inserts MR version validation even if no other MR checks are selected. On merge requests, directly changed projects with `require-bump: true` must increase their version over the fetched target branch and existing matching release tags. `require-bump: false` disables advancement checks. Push, scheduled, manual and API checks can retest an already released version. Release creation always checks that a new tag advances the version. New version files are permitted only when absent in the fetched baseline. Dynamic Python version providers are not supported.
...
Release workflows validate the tag, gate builds on checks, publish only explicitly selected packages, and emit a release-completion receipt. `release.gitlab: true` explicitly creates the GitLab Release entry using the token; it defaults to false. Image-only or package-only delivery does not require this API token. Projects sharing a release tag serialize their completion jobs to avoid racing the GitLab Release API. npm publication retries are not yet idempotent for an existing version; reconcile a partial publication before retrying. Enforce successful pipelines in repository merge settings.

## Buildah and air gap

Provide images.python, images.node, images.bun, images.helm and optionally images.control. Required role images are checked; there is no public fallback. Every image needs Python 3.11+, Git, Bash and a compatible 0.4.x toolkit because the runtime handles receipts and dispatch. Node/Bun images also need npm and Git. The builder image needs Buildah and Python; Helm image needs Helm, Git and Python. Runtime acquisition must be prepared offline, not installed in each consumer pipeline.
...
Deployments select a platform target with namespace, kubeconfig-variable, url, production and optional release name. Configure distinct targets for staging/production. MR deployments append project/MR identity and require same-project MRs. MR previews reject targets marked production, in both compilation and runtime. MR previews select every mapped image build to establish their own baseline. Complete deployments also select all mapped producers when affected; partial release deployments preserve unchanged images from their own existing baseline.
...
Checks and project/platform defaults accept a native `cache` mapping. Paths are repository-relative and must be named directories; repository root, `.ci-out`, `.git`, `.venv`, and `node_modules` are rejected to keep receipts and installed environments out of caches. Cache misses remain correct; frozen installation and artifact verification still run.
...
For Node, use a download directory such as `.cache/npm` with `NPM_CONFIG_CACHE: $CI_PROJECT_DIR/.cache/npm`. Keep registry credentials in CI variables or runtime configuration, never cached files. The toolkit does not implicitly share caches between projects.
